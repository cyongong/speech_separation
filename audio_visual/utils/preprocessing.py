...
def preprocess_sample(file, params):
    ...
    audioFile = file + ".wav"
    newaudio = file+"2.wav"
    new2audio = file+ "a.wav"
    roiFile = file + "2.png"
    visualFeaturesFile = file + "a.npy"
    ...
    v2aCommand = "ffmpeg -y -v quiet -i " + videoFile + " -ac 1 -ar 16000 -vn " + newaudio
    v2aCommand2 = "ffmpeg -ss 00:00:00 -i "+ newaudio +" -to 00:00:03 -c copy " + new2audio
    os.system(v2aCommand)
    os.system(v2aCommand2)

    count=0
    #for each frame, resize to 224x224 and crop the central 112x112 region
    captureObj = cv.VideoCapture(videoFile)
    roiSequence = list()
    while (captureObj.isOpened() and count!= 75):
        count+=1
